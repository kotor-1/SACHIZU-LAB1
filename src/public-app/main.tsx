...
function Home() {
  return <main className="public-home">
    ...
    <nav className="public-cards" aria-label="解析種目">
      {[['cmj', '01', 'CMJ', '垂直跳び', 'カメラ・録画から、跳躍高を推定。'], ['rj', '02', 'RJ', 'リバウンドジャンプ', '足元を自動判定して平均・最大RSIを推定。身長・時刻の手動入力は不要。'], ['sprint10', '03', '10m', 'スプリント', '2本のラインと最初の1歩を設定。通過時間・歩数・各歩の距離。']].map(([id, n, title, subtitle, text]) => <a key={id} href={`${home}?lab=${id}`}><span>{n} / {subtitle}</span><h2>{title}<b aria-hidden="true">↗</b></h2><p>{text}</p><strong>解析をはじめる →</strong></a>)}
    </nav>
