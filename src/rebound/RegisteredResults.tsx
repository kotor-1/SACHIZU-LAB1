...
export default function RegisteredResults({ result, seek, saveJSON, saveCSV, reviewed = false }: {
  ...
  const label = reviewed ? fullyConfirmed ? '確認済み' : '暫定・未確認含む' : '認識';
  const heightLabel = reviewed ? heightsConfirmed ? '確認済み' : '暫定・未確認含む' : '認識';
  return <section className="rj-registered" aria-label={reviewed ? '跳躍の集計結果（暫定値と確認済み値を区別）' : '3点登録の予測結果'}><h2>{result.jumps.length}跳躍の集計結果</h2>
    ...
    <div className="rj-template-metrics"><div>平均RSI（{label} {count} 回）<strong>{value(mean, 2)} <small>m/s</small></strong></div>
      <div>最大RSI（同じ {count} 回）<strong>{value(max, 2)} <small>m/s</small></strong></div><div>{reviewed ? '算出回数' : '採用回数'}<strong>{count} / {target}</strong><small>静止開始の1回目は除外</small></div>
      <div>平均跳躍高（{heightLabel} {heights.length} 回）<strong>{value(heights.length ? heights.reduce((a, b) => a + b, 0) / heights.length * 100 : null, 1)} <small>cm</small></strong></div></div>
