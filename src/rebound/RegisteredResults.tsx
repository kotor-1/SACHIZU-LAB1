...
export default function RegisteredResults({ result, seek, saveJSON, saveCSV }: {
  result: RegisteredAnalysis; seek: (pts: number) => void; saveJSON: () => void; saveCSV: () => void;
}) {
  const value = (n: number | null, digits = 3) => n === null ? '—' : n.toFixed(digits);
  const target = Math.max(0, result.jumps.length - 1);
  const complete = target > 0 && result.validRSICount === target;
  const heights = result.jumps.flatMap(j => j.heightM === null ? [] : [j.heightM]);
  return <section className="rj-registered" aria-label="3点登録の予測結果"><h2>{result.jumps.length}跳躍の集計結果</h2>
    ...
    <div className="rj-template-metrics"><div>平均RSI（認識 {result.validRSICount} 回）<strong>{value(result.meanRSI, 2)} <small>m/s</small></strong></div>
      <div>最大RSI（同じ {result.validRSICount} 回）<strong>{value(result.maxRSI, 2)} <small>m/s</small></strong></div><div>採用回数<strong>{result.validRSICount} / {target}</strong><small>静止開始の1回目は除外</small></div>
      <div>平均跳躍高（認識 {heights.length} 回）<strong>{value(heights.length ? heights.reduce((a, b) => a + b, 0) / heights.length * 100 : null, 1)} <small>cm</small></strong></div></div>
    {!complete && <p className="rj-warning">RSIを算出できた{result.validRSICount}回だけの平均・最大です。不成立を0で埋めません。採用回数・対象の異なる結果との比較には注意してください。</p>}
    <div className="rj-table-wrap"><table><caption>跳躍ごとの手動値・予測値</caption><thead><tr><th>跳躍</th><th>高さ cm</th><th>接地 秒</th><th>滞空 秒</th><th>RSI m/s</th><th>動画で確認</th></tr></thead><tbody>
      {result.jumps.map(j => <tr key={j.jump}><th>{j.jump}回目</th><td>{value(j.heightM === null ? null : j.heightM * 100, 1)}<small>時間からの推定</small></td>
        ...
        <td>{value(j.rsi, 2)}{j.reason && <small>{j.reason}</small>}</td><td>{j.takeoff && <button onClick={() => seek(j.takeoff!.pts)}>離地</button>} {j.landing && <button onClick={() => seek(j.landing!.pts)}>着地</button>}</td></tr>)}
    </tbody></table></div>
